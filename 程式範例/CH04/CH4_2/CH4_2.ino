...
//宣告0~9編碼
const int dispData[4]={5,6,7,8};    //宣告所要顯示的資料
...
void loop() {
  ...
  for(int i=0;i<5;i++) {      //閃爍顯示2秒
    for(int j=0;j<50;j++) {
      for(int k=0;k<4;k++) {     //顯示0.2秒
          digitalWrite(latchPin, 0);    //解除資料栓鎖
          shiftOut(dataPin, clockPin, MSBFIRST, segCode[dispData[k]]);
        //輸出顯示資料
          digitalWrite(latchPin, 1);    //栓鎖資料
          digitalWrite(scan[3-k], 0);   //輸出掃描信號
          delay(1);         //暫停1毫秒
          digitalWrite(scan[3-k], 1);   //關閉掃描信號
      }
    }
    delay(200);         //不顯示0.2秒
  }
